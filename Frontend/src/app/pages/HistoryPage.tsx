...
export function HistoryPage() {
  ...
  useEffect(() => {
    const savedHistory = localStorage.getItem('solarops_history');
    if (savedHistory) {
      try {
        const history: HistoryPanel[] = JSON.parse(savedHistory);
        setHistoryPanels(history);

        // Extract unique areas
        const uniqueAreas = [...new Set(history.map(panel => panel.area))];
