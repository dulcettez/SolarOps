...
export function PanelFarmViewPage() {
  ...
  const generateGrid = () => {
    ...
    const criticalCount = Math.max(1, Math.floor(totalPanels * 0.02));
    const mediumCount = Math.floor(totalPanels * 0.15);

    const criticalIndices = new Set<number>();
    while (criticalIndices.size < criticalCount) {
      criticalIndices.add(Math.floor(Math.random() * totalPanels));
    }
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        if (status === 'critical') {
          const defects = ['Hotspot Detected', 'Cell Cracking', 'Junction Box Failure'];
          const defectIndex = Math.floor(Math.random() * 3);
          panel.defectType = defects[defectIndex];
          panel.coverage = Math.floor(Math.random() * 30) + 15;
          panel.maxTemp = Math.floor(Math.random() * 20) + 60;
          panel.deltaT = Math.floor(Math.random() * 10) + 15;
          panel.financialLoss = Math.floor(Math.random() * 500) + 200;
          panel.crewType = 'repair';
          panel.energyLossPerDay = Math.floor(Math.random() * 3) + 2; // 2-5 kWh/day
          panel.costLossPerDay = Math.floor(Math.random() * 3) + 1.5; // 1.5-4.5 SAR/day
          panel.efficiencyLoss = Math.floor(Math.random() * 30) + 40; // 40-70%
          panel.peopleRequired = Math.min(Math.floor(Math.random() * 2) + 2, 3); // 2-3 people, max 3 (repair crew size)
          panel.maintenanceDecision = 'URGENT: Schedule repair crew within 24 hours. Panel poses fire risk and significant energy loss.';
          panel.priority = 'high';
        } else if (status === 'medium') {
          const defects = ['Soiling Detected', 'Minor Shading', 'Surface Degradation'];
          const defectIndex = Math.floor(Math.random() * 3);
          panel.defectType = defects[defectIndex];
          panel.coverage = Math.floor(Math.random() * 15) + 5;
          panel.maxTemp = Math.floor(Math.random() * 10) + 45;
          panel.deltaT = Math.floor(Math.random() * 5) + 5;
          panel.financialLoss = Math.floor(Math.random() * 150) + 50;
          panel.crewType = defectIndex === 0 ? 'cleaning' : 'repair';
          panel.energyLossPerDay = Math.floor(Math.random() * 1.5) + 0.5; // 0.5-2 kWh/day
          panel.costLossPerDay = Math.floor(Math.random() * 1) + 0.25; // 0.25-1.25 SAR/day
          panel.efficiencyLoss = Math.floor(Math.random() * 20) + 10; // 10-30%
          panel.peopleRequired = panel.crewType === 'cleaning'
            ? Math.min(Math.floor(Math.random() * 2) + 1, 5) // 1-2 people, max 5 (cleaning crew size)
            : Math.min(Math.floor(Math.random() * 1) + 1, 3); // 1-2 people, max 3 (repair crew size)
          panel.maintenanceDecision = panel.crewType === 'cleaning'
            ? 'Schedule cleaning crew within 7 days. Moderate efficiency loss detected.'
            : 'Schedule repair crew within 3-5 days. Minor structural issue detected.';
          panel.priority = 'medium';
        } else {
          panel.defectType = 'No Defects Detected';
          panel.coverage = 0;
          panel.maxTemp = Math.floor(Math.random() * 5) + 35;
          panel.deltaT = Math.floor(Math.random() * 3) + 1;
          panel.financialLoss = 0;
          panel.crewType = 'none';
          panel.energyLossPerDay = 0;
          panel.costLossPerDay = 0;
          panel.efficiencyLoss = 0;
          panel.peopleRequired = 0;
                ...
                <div className="bg-white border-b-2 border-[var(--solar-border)] p-6">
                  ...
                  >
                    ...
                    >
                      ...
                      {selectedPanel.status === 'critical' ? 'CRITICAL' : selectedPanel.status === 'medium' ? 'WARNING' : 'HEALTHY'}
                    </motion.div>
                  </motion.div>
                </div>
