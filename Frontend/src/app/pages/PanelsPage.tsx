...
export function PanelsPage() {
  ...
  // Get recommended action based on defect type
  const getRecommendedAction = (panel: Panel): string => {
    if (!panel.defectType) return 'Monitor performance';
    if (panel.defectType.includes('Hotspot')) return 'Inspect junction box immediately';
    if (panel.defectType.includes('Soiling')) return 'Schedule cleaning crew';
    if (panel.defectType.includes('Crack')) return 'Replace panel';
    if (panel.defectType.includes('Shading')) return 'Remove shading source';
    if (panel.defectType.includes('Degradation')) return 'Schedule repair crew';
    ...
    if (savedFarms) {
      try {
        ...
        farms.forEach(farm => {
          ...
          const defectivePanels = farm.grid.filter(panel => panel.status !== 'healthy');
